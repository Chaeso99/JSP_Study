...
<%
	...
	if(flag!=null){
		if(flag.equals("insert")){
			...
			cbean.setNum(num);//어떤 게시물 정보인지?
			cbean.setName(request.getParameter("cName"));//댓글 작성자
			cbean.setComment(request.getParameter("comment"));//댓글내용
			cmgr.insertBComment(cbean);
		}else if(flag.equals("delete")){
			cmgr.deleteBComment(UtilMgr.parseInt(request, "cnum"));
		}
	}else{
		//list.jsp 게시물 읽어옴:이때만 조회수 증가
		mgr.upCount(num);
	}
